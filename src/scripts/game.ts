...
function attachCalculation(row: HTMLTableRowElement) {
  ...
  const betSelect = row.querySelector<HTMLSelectElement>(
    `select[name="bet-${playerIndex}"]`
  )!;
  const takeSelect = row.querySelector<HTMLSelectElement>(
    `select[name="take-${playerIndex}"]`
  )!;
  const bonusesInput = row.querySelector<HTMLInputElement>(
